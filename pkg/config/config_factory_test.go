...
func TestGetCfgFactoryConfig(t *testing.T) {
	...
	t.Run("test_config_with_env_var_set", func(t *testing.T) {
		...
		receiver := cfg.Receivers[component.NewID(component.MustNewType("awsxray"))].(*awsxrayreceiver.Config)
...
func TestGetMapProviderContainer(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, cfg)
	extension := cfg.Extensions[component.NewID(component.MustNewType("pprof"))].(*pprofextension.Config)
	require.NotNil(t, extension)
	require.Equal(t, expectedEndpoint, extension.TCPAddr.Endpoint)

	require.NotNil(t, cfg.Receivers[component.NewID(component.MustNewType("otlp"))])
	require.NotNil(t, cfg.Exporters[component.NewID(component.MustNewType("awsemf"))])
}
